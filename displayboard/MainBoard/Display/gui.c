...
{
	Gui_DrawLine(x1,  y1,  x2,y1, fc); //H
	Gui_DrawLine(x1,  y1,  x1,y2, fc); //V
	
	Gui_DrawLine(x2,  y2,  x2,y1, fc); //H
	Gui_DrawLine(x2,  y2,  x1,y2, fc); //V
}
	...
	{	
		if( *s < 0x80 ) 
		{
			...
			for(i = 0;i < 16;i ++)
			{
				for(j = 0;j < 8;j ++)
				{
					//if(asc2_1608[k][2*i] & (0x80>>j))
					if(asc2_1608[k][i] & (0x80>>(7-j)))
					{
			...
			for (k=0;k<hz16_num;k++) 
			{
			  ...
			  { 
				    for(i=0;i<16;i++)
				    {
						for(j=0;j<8;j++) 
							{
						    	if(tfont16[k].Msk[i*2]&(0x80>>j))	Gui_DrawPoint(x+j,y+i,fc);
								else {
						...
						for(j=0;j<8;j++) 
							{
						    	if(tfont16[k].Msk[i*2+1]&(0x80>>j))	Gui_DrawPoint(x+j+8,y+i,fc);
								else 
								{
			...
			for (k=0;k<hz24_num;k++) 
			{
			  ...
			  { 
				    for(i=0;i<24;i++)
				    {
						...
						for(j=0;j<8;j++) 
							{
						    	if(tfont24[k].Msk[i*3+1]&(0x80>>j))	Gui_DrawPoint(x+j+8,y+i,fc);
								else {
									if (fc!=bc) Gui_DrawPoint(x+j+8,y+i,bc);
								}
							}
	...
	while(*s)
	{
		if( *s < 0x80 ) 
		{
			...
			for(i = 0;i < 64;i ++)
			{
				...
				for(j = 0;j < 8;j ++)
				{
					if(asc2_6432[k][4*i + 3] & (0x80>>j))
					{
						Gui_DrawPoint(x+j+24,y+i,fc);
					}
					else
					{
						Gui_DrawPoint(x+j+24,y+i,bc);
					}
				}
			}
...
//	unsigned short k;
	...
	for(n=0;n<4;n++)
	{
		for(i = 0;i < 128;i ++)
		{
			...
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 1] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+8,y+i,fc);
				}
				else
			...
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 2] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+16,y+i,fc);
				}
				else
			...
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 3] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+24,y+i,fc);
				}
				else
			...
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 4] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+32,y+i,fc);
				}
				else
				{
					Gui_DrawPoint(x+j+32,y+i,bc);
				}
			}
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 5] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+40,y+i,fc);
				}
				else
				{
					Gui_DrawPoint(x+j+40,y+i,bc);
				}
			}
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 6] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+48,y+i,fc);
				}
				else
				{
					Gui_DrawPoint(x+j+48,y+i,bc);
				}
			}
			for(j = 0;j < 8;j ++)
			{
				if(asc2_12864[*(s+n)][8*i + 7] & (0x80>>j))
				{
					Gui_DrawPoint(x+j+56,y+i,fc);
				}
				else
				{
					Gui_DrawPoint(x+j+56,y+i,bc);
				}
			}
		}
		x+=64;
	...
	for(j = 0;j < 48;j++)
	{
		for(i = 0;i < 6;i++)
		{
			for(k=0;k<8;k++)
			{
				if((*(p + (j*6 + i))) & (1 << (7-k)))
				{
					//Gui_DrawPoint(x+8*i+k,y+j,BLACK);
					Gui_DrawPoint(x+8*i+k,y+j,GRAY0);
				}
				else
				{
					//Gui_DrawPoint(x+8*i+k,y+j,GRAY0);
...
// 	   	for(j=0;j<3;j++)
// 		{	
// 			LCD_SetWindows(40*j+2,40*k,40*j+39,40*k+39);		//坐标设置
// 		    for(i=0;i<40*40;i++)
// 			 {	
// 			 	picL=*(p+i*2);	//数据低位在前
// 				picH=*(p+i*2+1);				
